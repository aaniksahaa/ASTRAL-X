...
#   * automatic mirroring by test-astralx-simulated.sh on real runs and on the
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/astralx-outputs-mirror-test.XXXXXX")"
...
[[ "$(astralx_default_simphy_outputs_dir "$DATA")" == "$OUTPUTS" ]] || \
  fail "standard simphy/data tree should mirror into outputs/simphy"
[[ "$(astralx_default_simphy_outputs_dir "${TMP}/other/data")" == "${TMP}/other/outputs" ]] || \
  fail "a non-simphy data dir should mirror into its 'outputs' sibling"
[[ "$(astralx_default_simphy_outputs_dir "${TMP}/custom")" == "${TMP}/custom_outputs" ]] || \
  fail "custom data dir should mirror into a '<name>_outputs' sibling"
RESOLVED="$(astralx_prepare_simphy_outputs_dir "" "$DATA")"
[[ "$RESOLVED" == "$OUTPUTS" && -d "$OUTPUTS" ]] || fail "default outputs dir was not created: $RESOLVED"
if astralx_prepare_simphy_outputs_dir "${DATA}/outputs" "$DATA" >/dev/null 2>"${TMP}/inside.err"; then
  fail "an outputs dir inside the data dir was accepted"
fi
grep -q "inside it" "${TMP}/inside.err" || fail "containment error was unclear"
if astralx_prepare_simphy_outputs_dir "${BASE}" "$DATA" >/dev/null 2>/dev/null; then
  fail "an outputs dir containing the data dir was accepted"
fi
if PHYLOGENY_DATA_DIR="$BASE" true && [[ "$(PHYLOGENY_DATA_DIR="$BASE" astralx_prepare_simphy_outputs_dir_standalone "")" != "$OUTPUTS" ]]; then
...
make_results() {
  ...
  : > "${results}/.astralx.lock"
  printf 'exit_code=0\n' > "${results}/.astralx.success"
...
for r in R1 R2; do
  ...
  make_results "${DATA}/${DS}/${r}/astralx_outputs/search-space_S1" "${r}-S1"
done
make_results "${DATA}/${DS}/R1/astralx_outputs/search-space_S2" "R1-S2"
make_results "${DATA}/${DS}/R1/aster_outputs/default" "R1-astralx"
...
make_results "${DATA}/${DS_INC}/R1/astralx_outputs/search-space_S1" "inc-R1-S1"
...
for leaf in R1/search-space_S1 R1/search-space_S2 R2/search-space_S1; do
  ...
  [[ -f "${M}/${leaf}/.astralx.lock" && -f "${M}/${leaf}/.astralx.success" ]] || fail "run markers not mirrored: $leaf"
done
[[ -s "${OUTPUTS}/aster_outputs/${DS}/R1/default/out-astralx.tre" ]] || fail "second method was not mirrored"
[[ -f "${OUTPUTS}/aster_outputs/${DS}/${DS}.command" ]] || fail "second method lacks the .command copy"
[[ -f "${OUTPUTS}/astralx_outputs/${DS_INC}/${DS}.command" ]] || fail "incomplete dataset lacks base .command"
[[ -s "${OUTPUTS}/astralx_outputs/${DS_INC}/R1/search-space_S1/out-astralx.tre" ]] || fail "incomplete results not mirrored"
[[ -z "$(astralx_simphy_find_forbidden_in_mirror "$OUTPUTS")" ]] || \
  fail "forbidden files leaked into the mirror: $(astralx_simphy_find_forbidden_in_mirror "$OUTPUTS")"
[[ ! -e "${OUTPUTS}/astralx_outputs/${DS}/1" ]] || fail "raw SimPhy replicate dir leaked into the mirror"
[[ -z "$(find "$OUTPUTS" -name 'stat-sim.csv' -print -quit)" ]] || fail "stat-sim.csv leaked into the mirror"
# The data tree keeps its exact layout.
[[ -f "${DATA}/${DS}/R1/astralx_outputs/search-space_S1/out-astralx.tre" && -f "${DATA}/${DS}/R1/all_gt.tre" ]] || \
...
rm -rf "${DATA}/${DS}/R2/astralx_outputs/bad"
...
SRC_LEAF="${RUN_DATA}/${RUN_DS}/R1/astralx_outputs/search-space_S1__cpu_true"
...
grep -q "^# setting:      search-space_S1__cpu_true$" "${RUN_LEAF}/out-astralx.command" || fail "command record lacks the setting"
diff -r "$SRC_LEAF" "$RUN_LEAF" >/dev/null || fail "mirror leaf differs from the results dir"
[[ -f "${RUN_OUTPUTS}/astralx_outputs/${RUN_DS}/${RUN_DS}.command" ]] || fail "run did not mirror the .command file"
[[ ! -e "${RUN_OUTPUTS}/astralx_outputs/${RUN_DS}/R1/all_gt.tre" && -z "$(astralx_simphy_find_forbidden_in_mirror "$RUN_OUTPUTS")" ]] || \
  fail "run mirror contains simulated input data"
[[ -f "${SRC_LEAF}/out-astralx.tre" && -f "${RUN_DATA}/${RUN_DS}/R1/all_gt.tre" ]] || fail "data tree layout changed"
...
grep -q "Run plan (2 dataset(s), 2 replicate(s), 2 setting(s))" "${TMP}/plan.out" || \
  fail "bulk dry-run summary is wrong: $(cat "${TMP}/plan.out")"
[[ "$(grep -c '^  t_.* / R1-R2 / search-space_' "${TMP}/plan.out")" == 4 ]] || \
  fail "bulk plan did not print one line per dataset/setting"
...
grep -Fq -- "--path-in-repo ph/d/simulated/outputs/astralx_outputs/${DS} " "${TMP}/up.out" || \
  fail "remote path for astralx outputs is wrong"
grep -Fq -- "--path-in-repo ph/d/simulated/outputs/aster_outputs/${DS} " "${TMP}/up.out" || \
  fail "remote path for astralx outputs is wrong"
grep -Fq -- "--path-in-repo ph/d/simulated/outputs/astralx_outputs/${DS_INC} " "${TMP}/up.out" || \
  fail "incomplete dataset was not planned"
grep -Fq -- "--local-path $(printf '%q' "${OUTPUTS}/astralx_outputs/${DS}") " "${TMP}/up.out" || fail "local path is wrong"
...
grep -q "uploaded=3 failed=0" "${TMP}/up-noyes.out" || fail "non-interactive stub upload did not complete"

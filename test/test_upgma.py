...
test_upgma.py
=============
Randomized tests for the ASTRAL-X UPGMA clusterer.
...
For each test case:
  ...
  4. Run ASTRAL-X with --verify-upgma → ASTRAL-X bipartitions.
...
Usage:
  python3 test/test_upgma.py [--astralx-root PATH] [--seeds N N N ...] [-n NUM] [-v]
"""
...
def first_seen_order(newick_lines):
    ...
    all Newick strings, exactly as ASTRAL-X's TaxonRegistry assigns IDs.
    """
    seen = {}
...
# ── Parse ASTRAL-X --verify-upgma output ─────────────────────────────────────

def parse_astralx_upgma(output):
    """
    Parse stdout from ASTRAL-X --verify-upgma.
...
def run_one(seed, n_taxa, k_trees, astralx_root, verbose):
    ...
    # Determine first-seen taxon order (mirrors ASTRAL-X TaxonRegistry)
    ...
    # Reorder sim matrix to first-seen ordering so tie-breaking matches ASTRAL-X
    n = len(ordered_taxa)
    sim = [[0.0] * n for _ in range(n)]
    for i in range(n):
        for j in range(n):
    ...
    if result.returncode != 0:
        if verbose:
            print(f"  ASTRAL-X stderr:\n{result.stderr[:500]}")
        return False, f"ASTRAL-X exited with code {result.returncode}"

    ax_bips = parse_astralx_upgma(result.stdout)
    ...
    if only_ax:
        msg.append(f"  in ASTRAL-X only: {[sorted(s) for s in only_ax]}")
    return False, "\n".join(msg)
...
def main():
    ap = argparse.ArgumentParser(description="Test ASTRAL-X UPGMA clusterer")
    ap.add_argument("--astralx-root", default=os.path.join(os.path.dirname(__file__), ".."),
                    help="ASTRAL-X project root (default: parent of test/)")
    ...
    astralx_root = os.path.abspath(args.astralx_root)
    ...
    for seed in seeds:
        ...
        ok, msg = run_one(seed, n_taxa, k_trees, astralx_root, args.verbose)
        if ok:
            passed += 1
            if args.verbose:

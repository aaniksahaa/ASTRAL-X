...
========================
20 randomized tests for the ASTRAL-X distance matrix computation.
...
For each test case:
  ...
  3. Run ASTRAL-X with --verify-distance-matrix and parse its output.
  4. Assert all distances match within tolerance.

Usage:
  python3 test/test_distance_matrix.py [--astralx-root PATH] [--mode cpu|gpu]
...
def run_astralx_distance_matrix(newick_lines, astralx_root, mode):
    """
    Write trees to a temp file, invoke ASTRAL-X --verify-distance-matrix,
...
def parse_dm_output(output):
    """
    Parse the DISTANCE_MATRIX block from ASTRAL-X stdout.
...
def run_test(seed, astralx_root, mode, verbose=False):
    ...
    # ASTRAL-X answer
    taxa_order, actual_raw = run_astralx_distance_matrix(newick_lines, astralx_root, mode)
...
def main():
    ap = argparse.ArgumentParser(description="Test ASTRAL-X distance matrix")

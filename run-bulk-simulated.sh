...
Options:
  ...
  --astralx-opts    Extra options for ASTRAL-X simulated runs
  --help, -h        Show this message

Examples:
  ./run-bulk-simulated.sh -m astralx
  ./run-bulk-simulated.sh -m astralx --num-replicates 3 --astralx-opts "--search-mode full -vv"
EOF
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --astralx-opts) ASTRAL_OPTS="$2"; shift 2 ;;
    --astralx-opts=*) ASTRAL_OPTS="${1#*=}"; shift ;;
        ...
        for spmax in "${SPMAX_LIST[@]}"; do
          ...
          for ((i=1; i<=NUM_REPLICATES; i++)); do
            ...
            if [[ -n "$ASTRAL_OPTS" ]]; then
              TEST_CMD+=(--astralx-opts "$ASTRAL_OPTS")
            fi
            "${TEST_CMD[@]}"
          done

/**
 * astralx_dp.cu — CUDA JNI for ASTRAL-X cross-tree DP transition discovery (Mode 2).
 *
...
#define CUDA_CHECK(call, ret) do { \
    cudaError_t _e = (call); \
    if (_e != cudaSuccess) { \
        fprintf(stderr, "[astralx_dp] CUDA error: %s  at %s:%d\n", \
...
#define RELEASE_AND_RETURN(ret) do { \
    ...
    {
        fprintf(stderr, "[astralx_dp] cudaMalloc failed for cluster data\n");
    ...
    {
        fprintf(stderr, "[astralx_dp] cudaMalloc failed for hash table\n");
        cudaFree(d_sums); cudaFree(d_xors); cudaFree(d_sizes);
    ...
    {
        fprintf(stderr, "[astralx_dp] cudaMalloc failed for per-round buffers\n");
        cudaFree(d_sums); cudaFree(d_xors); cudaFree(d_sizes);
        cudaFree(d_ht_sums); cudaFree(d_ht_xors); cudaFree(d_ht_sizes); cudaFree(d_ht_idx);

...
if [[ "$CHECK_ONLY" == true ]]; then
  ...
  if [[ -f "${ASTRALX_ROOT}/build/astralx/Main.class" ]]; then ok "Java build output"; else warn "Java build output is absent; run ./setup_dev.sh"; fi
  if [[ "$CPU_ONLY" != true && -x "$(command -v nvcc 2>/dev/null || true)" ]]; then
    if [[ -f "${ASTRALX_ROOT}/native/libastralx_weight.so" ]]; then ok "CUDA native libraries"; else warn "CUDA libraries are absent; run ./setup_dev.sh"; fi
  fi
  echo
  (( failures == 0 )) || exit 1
...
PYTHON_BIN="${VENV_DIR}/bin/python"
"$PYTHON_BIN" -m pip install --disable-pip-version-check -r "${ASTRALX_ROOT}/requirements-dev.txt"

if [[ "$BUILD_PROJECT" == true ]]; then
  "${ASTRALX_ROOT}/build.sh"
  if [[ "$CPU_ONLY" != true ]]; then
    if command -v nvcc >/dev/null 2>&1; then
      CUDA_ARCH="$CUDA_ARCH_VALUE" "${ASTRALX_ROOT}/build_native.sh"
    else
...
echo "  Run         : ./run.sh -i gene_trees.tre -o species_tree.tre --search-space S1"
echo "  Monitor     : ./run-astralx-with-monitor.sh -i gene_trees.tre -o species_tree.tre --search-space S1"

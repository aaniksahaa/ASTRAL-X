...
#
# Local layout (maintained by test-astralx-simulated.sh / sync-simulated-outputs.sh):
...
Examples:
  ./scripts/upload-bulk-simulated-outputs.sh --dry-run          # astralx only
  ./scripts/upload-bulk-simulated-outputs.sh --sync
  ./scripts/upload-bulk-simulated-outputs.sh --method aster     # a different method
  ./scripts/upload-bulk-simulated-outputs.sh --all-methods      # everything in the mirror
EOF
}
...
if [[ "$SYNC_FIRST" == true ]]; then
  DATA_DIR="$(astralx_prepare_simphy_data_dir "$(expand_home "$DATA_DIR")")" || exit 2
  OUTPUTS_DIR="$(astralx_prepare_simphy_outputs_dir "$OUTPUTS_DIR" "$DATA_DIR")" || exit 2
else
  OUTPUTS_DIR="$(astralx_prepare_simphy_outputs_dir_standalone "$OUTPUTS_DIR")" || exit 2
fi
...
if [[ "$SYNC_FIRST" == true ]]; then
  ...
  SYNC_CMD=("${SCRIPT_DIR}/scripts/sync-simulated-outputs.sh" --simphy-data-dir "$DATA_DIR" --simphy-outputs-dir "$OUTPUTS_DIR" --quiet)
...
echo "ASTRAL-X simulated outputs uploader"
...
echo "Selection:         taxa >= $MIN_TAXA, gene trees >= $MIN_GENE_TREES$(
  [[ ${#METHOD_FILTER[@]} -gt 0 ]] && printf ', methods: %s' "${!METHOD_FILTER[*]}"
...
while IFS= read -r -d '' method_path; do
  ...
  while IFS= read -r -d '' dataset_path; do
    ...
    if ! astralx_simphy_dataset_name_is_valid "$dataset_name"; then
...
for entry in "${SELECTED[@]}"; do
  ...
  astralx_simphy_dataset_name_is_valid "$dataset_name"
  taxa="${BASH_REMATCH[1]}"
  gene_trees="${BASH_REMATCH[2]}"
  ...
  forbidden="$(astralx_simphy_find_forbidden_in_mirror "$dataset_path" | head -n1)"

...
Optional:
  ...
  --opts "..."          Extra algorithm options passed to ./astralx (scripts/run.sh)
...
if [[ ! -x "${ASTRALX_ROOT}/astralx" ]]; then
  echo -e "${RED}Error: astralx launcher not found or not executable in '$ASTRALX_ROOT'.${NC}"
  exit 1
fi
...
write_command_record() {
  ...
  fi
  {
    ...
    echo "cd $(printf '%q' "$ASTRALX_ROOT") && $(quote_command ./astralx --input "$INPUT_FILE" --output "$OUTPUT_FILE" "${ASTRALX_ARGS[@]}")"
...
if [[ "$TIME_MONITOR" == true && -n "$TIME_CMD" ]]; then
  (
    cd "$ASTRALX_ROOT" && "$TIME_CMD" -v ./astralx --input "$INPUT_FILE" --output "$OUTPUT_FILE" "${ASTRALX_ARGS[@]}" < /dev/null 2>&1 | tee "$TIME_TMP"
  ) &
  ASTRALX_PID=$!
else
  (
    cd "$ASTRALX_ROOT" && ./astralx --input "$INPUT_FILE" --output "$OUTPUT_FILE" "${ASTRALX_ARGS[@]}" < /dev/null 2>&1 | tee "$TIME_TMP"
  ) &
  ASTRALX_PID=$!
fi
...
if [[ -n "$REFERENCE_SPECIES_TREE" && -f "$OUTPUT_FILE" ]]; then
  if [[ -f "$REFERENCE_SPECIES_TREE" ]]; then
    rf_output=$(cd "$ASTRALX_ROOT" && "$PYTHON_BIN" scripts/rf.py "$OUTPUT_FILE" "$REFERENCE_SPECIES_TREE" 2>&1) || true

...
# ASTRAL-X runner
# ===============
# Usage: ./astralx -i <gene_trees> -o <output> [options]
#        (the repository-root launcher forwards here; scripts/run.sh works too)
#
...
# Every crash artifact (Java fatal report, HotSpot hs_err file) lands here; the
# directory is git-ignored.
CRASH_LOG_DIR="${ASTRALX_ROOT}/crash-logs"
...
Optional:
  ...
  --keep-polytomy-during-inference
                     ...
                     final quartet scoring always preserves input polytomies
...
Compatibility:
  Positional form './astralx <input> <output> ...' is also accepted.
EOF
}
...
# Backward-compatible positional form: ./astralx input output [opts...]
if [[ "${1:-}" != -* ]]; then
  INPUT_FILE="$1"
  shift
  if [[ $# -gt 0 && "${1:-}" != -* ]]; then
...
CRASH_LOG_OPTS=(
  -Dastralx.crashLogDir="${CRASH_LOG_DIR}"
  -XX:ErrorFile="${CRASH_LOG_DIR}/astralx-hotspot-crash-%p.log"
)

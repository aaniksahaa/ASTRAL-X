#!/usr/bin/env bash
# Extract union/intersection taxon names using ASTRAL-X's own Newick tokenizer.
...
if [[ ! -f "$ASTRALX_ROOT/build/astralx/Main.class" ]]; then
  echo "Error: compiled ASTRAL-X classes are missing; run ./scripts/build.sh first." >&2
  exit 2
fi
...
exec java -cp "$ASTRALX_ROOT/build" astralx.Main "${ARGS[@]}"

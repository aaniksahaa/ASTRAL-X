...
#
# New runs mirror themselves automatically (test-astralx-simulated.sh); this tool
...
Examples:
  ...
  ./scripts/sync-simulated-outputs.sh --method astralx
EOF
}
...
DATA_DIR="$(astralx_prepare_simphy_data_dir "$DATA_DIR")" || exit 2
...
echo "ASTRAL-X simulated outputs mirror sync"
...
while IFS= read -r -d '' results_dir; do
  mapfile -t parts < <(astralx_simphy_results_components "$DATA_DIR" "$results_dir" 2>/dev/null)
  if [[ ${#parts[@]} -ne 4 ]]; then
  ...
  if mirrored_path="$(astralx_mirror_simulated_results "$DATA_DIR" "$OUTPUTS_DIR" "$results_dir")"; then
    ((mirrored++)) || true
...
done < <(astralx_list_simulated_results_dirs "$DATA_DIR")

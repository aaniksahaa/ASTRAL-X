...
source "${SCRIPT_ROOT}/scripts/experiment-setting-name.sh"
...
Examples:
  ./scripts/test-astralx-simulated.sh -t 100 -g 100 -r R1 --fresh
  ./scripts/test-astralx-simulated.sh -t 100 -g 100 -r R1 --fresh --opts "--search-space S1 --intersection-method I2 -vv"
...
if [[ ! -f "$ALL_GT_FILE" ]]; then
  ...
  if [[ "$INCOMPLETE" == true ]]; then
    ...
    SIM_INC_CMD=("${ASTRALX_ROOT}/scripts/sim_incomplete.sh" -t "$TAXA_NUM" -g "$GENE_TREES" -rs "$REPLICATE_COUNT" --sb "$SB" --spmin "$SPMIN" --spmax "$SPMAX")
    ...
    SIM_CMD=("${ASTRALX_ROOT}/scripts/sim.sh" -t "$TAXA_NUM" -g "$GENE_TREES" -r "$REPLICATE" -rs "$REPLICATE_COUNT" --sb "$SB" --spmin "$SPMIN" --spmax "$SPMAX")
...
CMD=("${ASTRALX_ROOT}/scripts/run-astralx-with-monitor.sh" -i "$ALL_GT_FILE" -o "$OUT_ASTRALX" --astralx-root "$ASTRALX_ROOT" --no-notify)
...
if [[ -f "$OUT_ASTRALX" && -f "$TRUE_SPECIES_TREE" ]]; then
  rf_output=$("$PYTHON_BIN" "${ASTRALX_ROOT}/scripts/rf.py" "$OUT_ASTRALX" "$TRUE_SPECIES_TREE" 2>&1) || true
...
# the exact ./astralx invocation) with the outer commands that produced this run.

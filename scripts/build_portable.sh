#!/usr/bin/env bash
# Build a self-contained ASTRAL-X application image and transport archive.
...
case "$OS_RAW" in
  ...
  *) echo "Error: $OS_RAW is not supported by this script; use scripts/build_portable.ps1 on Windows." >&2; exit 1 ;;
esac
case "$ARCH_RAW" in
...
if [[ "$PLATFORM_OS" == "macos" ]]; then
  ...
  RAW_IMAGE="${JPACKAGE_OUT}/astralx.app"
  IMAGE="${WORK}/${ARTIFACT}"
  mkdir -p "$IMAGE"
...
ASTRAL-X ${VERSION} — self-contained ${PLATFORM_OS}-${PLATFORM_ARCH} build
...
[[ "$VERSION_OUTPUT" == *"ASTRAL-X  v${VERSION}"* \

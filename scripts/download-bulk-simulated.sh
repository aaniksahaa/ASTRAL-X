#!/usr/bin/env bash
# Download simulated ASTRAL-X datasets from Hugging Face.
#
...
Examples:
  ...
  ./scripts/download-bulk-simulated.sh --keep-zip --local-dir ~/phylogeny/ASTRAL-X/simphy/data
EOF
}
...
if [[ -e "$LOCAL_DATA_DIR" && ! -d "$LOCAL_DATA_DIR" ]]; then
  echo "Error: local data path exists but is not a directory: $LOCAL_DATA_DIR" >&2
  exit 2
fi
...
if [[ "$DRY_RUN" == false ]]; then
  mkdir -p "$LOCAL_DATA_DIR" || exit 2
  WORK_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/astralx-download.XXXXXX")" || exit 2
fi
...
echo "ASTRAL-X simulated dataset downloader"

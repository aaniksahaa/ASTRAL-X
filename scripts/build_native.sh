...
NVCC_FLAGS=(
  ...
  -DASTRALX_MIN_CUDA_CC="${MIN_CUDA_CC}"
)
...
OUT_W="$NATIVE_OUT_DIR/libastralx_weight.so"
...
OUT_DP="$NATIVE_OUT_DIR/libastralx_dp.so"
...
OUT_DM="$NATIVE_OUT_DIR/libastralx_dist.so"
...
OUT_SIM="$NATIVE_OUT_DIR/libastralx_sim.so"
...
echo "Run with:"
echo "  ./astralx -i <input.tre> -o <output.tre> --gpu --search-space S2 -vv --no-build"

...
bash "${SCRIPT_DIR}/scripts/sim.sh" "${SIM_ARGS[@]}"
echo
...
for i in $(seq 1 "${REPLICATES}"); do
  ...
  # Copy true species tree so test-astralx-simulated.sh --incomplete can compute RF
...
if [[ -d "$INCOMPLETE_DIR" ]]; then
  ...
  INCOMPLETE_COMMAND_TMP="${INCOMPLETE_COMMAND_FILE}.tmp.$$"
  {
    ...
    printf '%q -t %q -g %q -rs %q --sb %q --spmin %q --spmax %q --fraction %q --seed %q --min-keep %q\n' \
      "${SCRIPT_DIR}/scripts/sim_incomplete.sh" "$TAXA_NUM" "$GENE_TREES" "$REPLICATES" \

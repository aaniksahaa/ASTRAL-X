...
# Supports ASTRAL-X and optional baseline algorithms
...
ALG_COMMANDS["astralx"]="astralx_command"
...
source "${RUNNER_ROOT}/scripts/experiment-setting-name.sh"
...
normalize_algorithm_name() {
    local name="${1,,}"
    case "$name" in
      astralx|astral-x|stelar|stelar-x) echo "astralx" ;;
      aster) echo "aster" ;;
      astral) echo "astral" ;;
        ...
        case "$alg" in
          astralx)
            if [[ ! -x "$RUN_WITH_MONITOR_SCRIPT" ]]; then
              echo -e "${RED}Error: ASTRAL-X monitor wrapper not found: $RUN_WITH_MONITOR_SCRIPT${NC}"
              errors_found=true
            fi
            if [[ ! -x "${STELAR_X_ROOT}/astralx" ]]; then
              echo -e "${RED}Error: ASTRAL-X launcher ./astralx not found in project root.${NC}"
              errors_found=true
            fi
            ;;
...
if [[ -n "$METHODS_ARG" ]]; then
  ...
  for local_method in "${user_methods_raw[@]}"; do
    normalized_method=$(normalize_algorithm_name "$local_method") || {
      ...
      echo "Supported methods: astralx, aster, astral, treeqmc, wqfmtree, supertriplets, stp-nni, tmc"
      exit 1
    }
...
if [[ -z "${RUN_WITH_MONITOR_SCRIPT}" ]]; then
  RUN_WITH_MONITOR_SCRIPT="${STELAR_X_ROOT%/}/scripts/run-astralx-with-monitor.sh"
fi
...
echo "Output directories: astralx_outputs, aster_outputs, astral_outputs, treeqmc_outputs, wqfmtree_outputs, supertriplets_outputs, stp-nni_outputs, tmc_outputs"

...
Options:
  --no-sync                Do not refresh the mirror first (default with
                           --data-dir: ./scripts/sync-a10k-outputs.sh runs first, so
...
Examples:
  ...
  ./scripts/upload-a10k-outputs.sh --data-dir \$PHYLOGENY_DATA_DIR/10k-astral-dataset
  ./scripts/upload-a10k-outputs.sh --data-dir \$PHYLOGENY_DATA_DIR/10k-astral-dataset --no-sync
  ./scripts/upload-a10k-outputs.sh --outputs-dir \$PHYLOGENY_DATA_DIR/outputs/10k-astral-dataset --yes
EOF
}
...
if [[ "$SYNC_FIRST" == true ]]; then
  ...
  SYNC_CMD=("${SCRIPT_DIR}/scripts/sync-a10k-outputs.sh" --data-dir "$DATA_DIR" --outputs-dir "$OUTPUTS_DIR" --quiet)
...
if [[ ${#UPLOAD_DIRS[@]} -eq 0 && ${#ROOT_FILES[@]} -eq 0 ]]; then
  ...
  echo "Run ./scripts/sync-a10k-outputs.sh --data-dir ... (or this tool with --sync) to populate the mirror."
  exit 0
fi

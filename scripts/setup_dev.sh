#!/usr/bin/env bash
# Prepare and verify a Linux ASTRAL-X development checkout.
set -euo pipefail

ASTRALX_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
VENV_DIR="${ASTRALX_ROOT}/.venv"
CPU_ONLY=false
CHECK_ONLY=false
BUILD_PROJECT=true
RUN_TESTS=true
...
echo "=== ASTRAL-X developer environment ==="
echo "Repository: $ASTRALX_ROOT"
echo
echo "Required tools"
...
if [[ -x "${ASTRALX_ROOT}/simphy/simphy_lnx64" ]]; then ok "Bundled SimPhy executable"; else warn "Bundled SimPhy executable is missing or not executable"; fi
...
if [[ "$CHECK_ONLY" == true ]]; then
  ...
  if [[ -f "${ASTRALX_ROOT}/build/astralx/Main.class" ]]; then ok "Java build output"; else warn "Java build output is absent; run ./scripts/setup_dev.sh"; fi
  if [[ "$CPU_ONLY" != true && -x "$(command -v nvcc 2>/dev/null || true)" ]]; then
    if [[ -f "${ASTRALX_ROOT}/native/libastralx_weight.so" ]]; then ok "CUDA native libraries"; else warn "CUDA libraries are absent; run ./scripts/setup_dev.sh"; fi
  fi
  echo
  (( failures == 0 )) || exit 1
...
PYTHON_BIN="${VENV_DIR}/bin/python"
"$PYTHON_BIN" -m pip install --disable-pip-version-check -r "${ASTRALX_ROOT}/scripts/requirements-dev.txt"

if [[ "$BUILD_PROJECT" == true ]]; then
  "${ASTRALX_ROOT}/scripts/build.sh"
  if [[ "$CPU_ONLY" != true ]]; then
    if command -v nvcc >/dev/null 2>&1; then
      CUDA_ARCH="$CUDA_ARCH_VALUE" "${ASTRALX_ROOT}/scripts/build_native.sh"
    else
...
if [[ "$RUN_TESTS" == true ]]; then
  bash "${ASTRALX_ROOT}/test/run_cli_tests.sh"
  bash "${ASTRALX_ROOT}/test/run_tests.sh" --cpu --no-build
fi

echo
echo "Developer setup complete."
echo "  Check later : ./scripts/setup_dev.sh --check"
echo "  Run         : ./astralx -i gene_trees.tre -o species_tree.tre --search-space S1"
echo "  Monitor     : ./scripts/run-astralx-with-monitor.sh -i gene_trees.tre -o species_tree.tre --search-space S1"

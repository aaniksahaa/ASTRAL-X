 ...
 * 
 * Problem: Given two arrays A and B, count elements in A[l1..r1] ∩ B[l2..r2]
 * 
 ...
 * ============================================================================
 * 
 * 1. PERMUTATIONS OF THE SAME SET:
 *    - A and B must both be permutations of the SAME set S of values
 *    - |A| = |B| = |S| = n
 *    - Every value in S appears exactly once in A and exactly once in B
 * 
 * 2. NO DUPLICATES:
 *    - Each value must appear exactly once in each array
 *    - Duplicates would make "position of value v in B" ambiguous
 * 
 * 3. VALUE RANGE:
 ...
 *      • {0, 1, 2, ..., n-1}  (0-indexed)
 *      • {1, 2, 3, ..., n}    (1-indexed)  
 *      • Any set of n distinct non-negative integers
 *    - Internally, values are mapped to positions [0, n-1]
 * 
 * 4. ARRAY SIZE:
 *    - Both arrays must have the same length n
 *    - n can be any positive integer (not limited to powers of 2)
 *    - Wavelet matrix uses ceil(log2(n)) levels
 * 
 * EXAMPLES OF VALID INPUT:
 *   A = [2, 0, 3, 1, 4]     B = [4, 2, 1, 0, 3]   ✓ (permutations of {0,1,2,3,4})
 *   A = [5, 3, 1, 4, 2]     B = [2, 5, 4, 1, 3]   ✓ (permutations of {1,2,3,4,5})
 *   A = [10, 20, 30]        B = [30, 10, 20]      ✓ (permutations of {10,20,30})
 * 
 * EXAMPLES OF INVALID INPUT:
 *   A = [1, 2, 2, 3]        B = [3, 1, 2, 2]      ✗ (duplicates)
 *   A = [1, 2, 3]           B = [4, 5, 6]         ✗ (different value sets)
 *   A = [-1, 0, 1]          B = [1, -1, 0]        ✗ (negative values)
 *   A = [1, 2, 3]           B = [1, 2]            ✗ (different lengths)
 * 
 * ============================================================================
 */
...
// ============================================================
// Helper: Build Y array from permutations A and B
// ============================================================

/**
 * Builds the derived array Y where Y[p] = position of A[p] in B.
 * Uses hash map internally, so works for ANY value set (not just 0..n-1 or 1..n).
 * 
 * @param A First permutation array
 * @param B Second permutation array  
 * @param Y Output array (will be resized)
 * @return true if valid permutations, false if constraints violated
 */
bool buildDerivedArray(const std::vector<int>& A, const std::vector<int>& B, std::vector<int>& Y) {
    int n = A.size();
    
    // Check: same length
    if (B.size() != (size_t)n) {
        std::cerr << "Error: A and B have different lengths (" << n << " vs " << B.size() << ")\n";
        return false;
    }
    
    // Build position map for B: value -> position
    std::unordered_map<int, int> posB;
    for (int i = 0; i < n; i++) {
        if (B[i] < 0) {
            std::cerr << "Error: Negative value " << B[i] << " in B at position " << i << "\n";
            return false;
        }
        if (posB.count(B[i])) {
            std::cerr << "Error: Duplicate value " << B[i] << " in B\n";
            return false;
        }
        posB[B[i]] = i;
    }
    ...
    for (int i = 0; i < n; i++) {
        if (A[i] < 0) {
            std::cerr << "Error: Negative value " << A[i] << " in A at position " << i << "\n";
            return false;
        }
        auto it = posB.find(A[i]);
        if (it == posB.end()) {
            std::cerr << "Error: Value " << A[i] << " in A is not present in B\n";
            return false;
        }
        Y[i] = it->second;  // Y[i] = position of A[i] in B
    }
...
/**
 * Validates that A and B are valid permutations of the same set.
 * Returns detailed error message if invalid.
 */
std::string validatePermutations(const std::vector<int>& A, const std::vector<int>& B) {
    int n = A.size();
    ...
    if (setA.size() != (size_t)n) {
        return "Array A contains duplicates";
    }
    if (setB.size() != (size_t)n) {
        return "Array B contains duplicates";
    }
    if (setA != setB) {
        return "A and B are permutations of different value sets";
    }
    
    for (int v : A) {
        if (v < 0) return "Array A contains negative values";
    }
    for (int v : B) {
        if (v < 0) return "Array B contains negative values";
    }
...
__device__ __forceinline__ int rank1(const uint64_t* bitvector, const int* block_ranks, int pos, int n_limit) {
    if (pos <= 0) return 0;
...
// Rank of 0s
__device__ __forceinline__ int rank0(const uint64_t* bitvector, const int* block_ranks, int pos, int n_limit) {
    if (pos > n_limit) pos = n_limit;
    return pos - rank1(bitvector, block_ranks, pos, n_limit);
}

// Get bit at position
__device__ __forceinline__ int getBit(const uint64_t* bitvector, int pos) {
    return (bitvector[pos / 64] >> (pos % 64)) & 1;
}

// Count values < x in Y[l1..r1] using wavelet matrix
// This is a proper __device__ function, not a lambda (portable CUDA C++)
__device__ __forceinline__ int countLessThan(
    int x,
    int l1, int r1,
    ...
    // Use uint32_t to avoid overflow for levels up to 31
    // sigma = 2^levels is the alphabet upper bound
    uint32_t sigma = 1u << levels;
    
    // If x >= sigma, all values in range are < x
    if ((uint32_t)x >= sigma) return (r1 - l1 + 1);
    ...
    for (int level = levels - 1; level >= 0; level--) {
        ...
        if (bit == 0) {
            ...
            // For "count < x", if bit=0, we only go left and narrow the range
...
__global__ void waveletRangeCountKernel(
    ...
    // Handle malformed query: empty value range
    if (lo > hi) {
        results[qid] = 0;
        return;
    }
    ...
    int count_less_than_hi_plus_1 = countLessThan(hi + 1, l1, r1, bitvectors, block_ranks, zeros, 
                                                   n, levels, words_per_level, blocks_per_level);
    int count_less_than_lo = countLessThan(lo, l1, r1, bitvectors, block_ranks, zeros,
                                            n, levels, words_per_level, blocks_per_level);
...
int main() {
    ...
    // Timing data collection for final summary
    struct TimingData {
        double build_time_ms = 0;
        double gpu_batch_query_time_ms = 0;
        int batch_query_count = 0;
        double cpu_per_query_ms = 0;
        double memory_mb = 0;
        int levels = 0;
        
        // Per-range-size timings (from Demo 4)
        std::vector<std::tuple<int, double, int>> range_timings;  // (range_size, time_us, result)
    } timing;
    ...
    double memory_mb = (wm.levels * wm.words_per_level * 8 + 
                        wm.levels * wm.blocks_per_level * 4 +
                        wm.levels * 4) / (1024.0 * 1024.0);
    std::cout << "Memory: ~" << memory_mb << " MB\n\n";
    
    // Store timing data for final summary
    timing.build_time_ms = build_time;
    timing.memory_mb = memory_mb;
    timing.levels = wm.levels;
    ...
    timing.cpu_per_query_ms = cpu_time / cpu_sample;
    ...
    timing.gpu_batch_query_time_ms = gpu_time;
    timing.batch_query_count = NUM_QUERIES;
    ...
    for (auto& sz : range_sizes) {
        ...
        timing.range_timings.push_back(std::make_tuple(r1_size, time_us, gpu_result));
    }
    ...
    for (int test_n : test_sizes) {
        ...
        // Test 4: Empty range (lo > hi should return 0)
        {
            ...
            // With our fix: lo > hi should now return 0 (empty range handled correctly)
            bool pass = (result == 0);
            std::cout << "   Test 4 - Empty value range [" << (test_n-1) << ",0] (lo > hi): "
                      << result << " (expected 0)" << (pass ? " ✓" : " ✗") << "\n";
            if (pass) all_passed++;
            total_tests++;
        }
    ...
    // ========================================
    // GENERALIZED INPUT TESTING
    // ========================================
    std::cout << "\n========================================\n";
    std::cout << "GENERALIZED INPUT TESTING\n";
    std::cout << "========================================\n";
    std::cout << "Testing that the algorithm works for various value sets,\n";
    std::cout << "not just {1..n}.\n\n";
    
    int gen_passed = 0, gen_total = 0;
    
    // Test 1: 0-indexed permutation {0, 1, ..., n-1}
    {
        std::cout << "--- Test: 0-indexed permutation {0..n-1} ---\n";
        std::vector<int> testA = {2, 0, 4, 1, 3};  // permutation of {0,1,2,3,4}
        std::vector<int> testB = {4, 2, 1, 0, 3};
        
        std::string err = validatePermutations(testA, testB);
        if (!err.empty()) {
            std::cout << "   Validation failed: " << err << " ✗\n";
        } else {
            std::vector<int> testY;
            if (buildDerivedArray(testA, testB, testY)) {
                std::cout << "   A = [2, 0, 4, 1, 3]\n";
                std::cout << "   B = [4, 2, 1, 0, 3]\n";
                std::cout << "   Y = [";
                for (int i = 0; i < (int)testY.size(); i++) std::cout << testY[i] << (i < (int)testY.size()-1 ? ", " : "");
                std::cout << "]\n";
                
                // Build wavelet matrix and test
                WaveletMatrix testWM;
                buildWaveletMatrix(testY, testWM);
                
                int* d_q, *d_r;
                CUDA_CHECK(cudaMalloc(&d_q, 4 * sizeof(int)));
                CUDA_CHECK(cudaMalloc(&d_r, sizeof(int)));
                
                // Query: A[0..4] ∩ B[0..4] should be 5 (all elements)
                std::vector<int> q = {0, 4, 0, 4};
                CUDA_CHECK(cudaMemcpy(d_q, q.data(), 4 * sizeof(int), cudaMemcpyHostToDevice));
                waveletRangeCountKernel<<<1, 1>>>(
                    testWM.d_bitvectors, testWM.d_block_ranks, testWM.d_zeros,
                    testWM.n, testWM.levels, testWM.words_per_level, testWM.blocks_per_level,
                    d_q, d_r, 1);
                int result;
                CUDA_CHECK(cudaMemcpy(&result, d_r, sizeof(int), cudaMemcpyDeviceToHost));
                
                bool pass = (result == 5);
                std::cout << "   Query A[0..4] ∩ B[0..4]: " << result << " (expected 5)" << (pass ? " ✓" : " ✗") << "\n";
                if (pass) gen_passed++;
                gen_total++;
                
                // Query: A[1..3] ∩ B[2..4] 
                // A[1..3] = {0, 4, 1}, B[2..4] has positions 2,3,4 with values {1, 0, 3}
                // Intersection: {0, 1} -> count = 2
                q = {1, 3, 2, 4};
                CUDA_CHECK(cudaMemcpy(d_q, q.data(), 4 * sizeof(int), cudaMemcpyHostToDevice));
                waveletRangeCountKernel<<<1, 1>>>(
                    testWM.d_bitvectors, testWM.d_block_ranks, testWM.d_zeros,
                ...
                int cpu_result = 0;
                std::set<int> inB;
                for (int i = 2; i <= 4; i++) inB.insert(testB[i]);
                for (int i = 1; i <= 3; i++) if (inB.count(testA[i])) cpu_result++;
                
                pass = (result == cpu_result);
                std::cout << "   Query A[1..3] ∩ B[2..4]: " << result << " (expected " << cpu_result << ")" << (pass ? " ✓" : " ✗") << "\n";
                if (pass) gen_passed++;
                gen_total++;
                
                CUDA_CHECK(cudaFree(d_q));
                CUDA_CHECK(cudaFree(d_r));
                freeWaveletMatrix(testWM);
            }
        }
    }
    
    // Test 2: Arbitrary non-contiguous values {10, 20, 30, 40, 50}
    {
        std::cout << "\n--- Test: Non-contiguous values {10,20,30,40,50} ---\n";
        std::vector<int> testA = {30, 10, 50, 20, 40};
        std::vector<int> testB = {50, 30, 20, 10, 40};
        ...
        if (!err.empty()) {
            ...
            std::vector<int> testY;
            if (buildDerivedArray(testA, testB, testY)) {
                std::cout << "   A = [30, 10, 50, 20, 40]\n";
                std::cout << "   B = [50, 30, 20, 10, 40]\n";
                std::cout << "   Y = [";
                for (int i = 0; i < (int)testY.size(); i++) std::cout << testY[i] << (i < (int)testY.size()-1 ? ", " : "");
                std::cout << "]  (Y[i] = position of A[i] in B)\n";
                ...
                // Partial query: A[0..2] ∩ B[1..3]
                // A[0..2] = {30, 10, 50}, B[1..3] = {30, 20, 10}
                // Intersection: {30, 10} = 2
                q = {0, 2, 1, 3};
                CUDA_CHECK(cudaMemcpy(d_q, q.data(), 4 * sizeof(int), cudaMemcpyHostToDevice));
                waveletRangeCountKernel<<<1, 1>>>(
                    testWM.d_bitvectors, testWM.d_block_ranks, testWM.d_zeros,
                ...
                int cpu_result = 0;
                std::set<int> inB;
                for (int i = 1; i <= 3; i++) inB.insert(testB[i]);
                for (int i = 0; i <= 2; i++) if (inB.count(testA[i])) cpu_result++;
                
                pass = (result == cpu_result);
                std::cout << "   Query A[0..2] ∩ B[1..3]: " << result << " (expected " << cpu_result << ")" << (pass ? " ✓" : " ✗") << "\n";
                if (pass) gen_passed++;
                gen_total++;
    ...
    // Test 3: Larger sparse values {100, 500, 1000, 5000, 10000, 50000}
    {
        std::cout << "\n--- Test: Sparse values {100,500,1000,5000,10000,50000} ---\n";
        std::vector<int> testA = {1000, 100, 50000, 500, 10000, 5000};
        std::vector<int> testB = {500, 10000, 100, 50000, 5000, 1000};
        
        std::vector<int> testY;
        if (buildDerivedArray(testA, testB, testY)) {
            std::cout << "   A = [1000, 100, 50000, 500, 10000, 5000]\n";
            std::cout << "   B = [500, 10000, 100, 50000, 5000, 1000]\n";
            std::cout << "   Y = [";
            for (int i = 0; i < (int)testY.size(); i++) std::cout << testY[i] << (i < (int)testY.size()-1 ? ", " : "");
            std::cout << "]\n";
            ...
            // Random queries with CPU verification
            std::mt19937 test_rng(123);
            int correct = 0;
            int num_queries = 20;
            for (int i = 0; i < num_queries; i++) {
                int l1 = test_rng() % 6, r1 = test_rng() % 6;
                int l2 = test_rng() % 6, r2 = test_rng() % 6;
                if (l1 > r1) std::swap(l1, r1);
                if (l2 > r2) std::swap(l2, r2);
                
                std::vector<int> q = {l1, r1, l2, r2};
                CUDA_CHECK(cudaMemcpy(d_q, q.data(), 4 * sizeof(int), cudaMemcpyHostToDevice));
                waveletRangeCountKernel<<<1, 1>>>(
                    testWM.d_bitvectors, testWM.d_block_ranks, testWM.d_zeros,
                    testWM.n, testWM.levels, testWM.words_per_level, testWM.blocks_per_level,
                    d_q, d_r, 1);
                int gpu_result;
                CUDA_CHECK(cudaMemcpy(&gpu_result, d_r, sizeof(int), cudaMemcpyDeviceToHost));
                
                // CPU
                int cpu_result = 0;
                std::set<int> inB;
                for (int j = l2; j <= r2; j++) inB.insert(testB[j]);
                for (int j = l1; j <= r1; j++) if (inB.count(testA[j])) cpu_result++;
                
                if (gpu_result == cpu_result) correct++;
            }
            
            bool pass = (correct == num_queries);
            std::cout << "   Random queries: " << correct << "/" << num_queries << " correct" << (pass ? " ✓" : " ✗") << "\n";
            if (pass) gen_passed++;
            gen_total++;
    ...
    // Test 4: Validation should REJECT invalid inputs
    {
        std::cout << "\n--- Test: Input validation (should reject invalid inputs) ---\n";
        
        // Duplicates
        std::vector<int> dupA = {1, 2, 2, 3};
        std::vector<int> dupB = {3, 1, 2, 2};
        std::string err = validatePermutations(dupA, dupB);
        bool pass = !err.empty();
        std::cout << "   Duplicates [1,2,2,3]: " << (pass ? "Rejected ✓" : "Accepted ✗") << " (" << err << ")\n";
        if (pass) gen_passed++;
        gen_total++;
        
        // Different sets
        std::vector<int> diffA = {1, 2, 3};
        std::vector<int> diffB = {4, 5, 6};
        err = validatePermutations(diffA, diffB);
        pass = !err.empty();
        std::cout << "   Different sets {1,2,3} vs {4,5,6}: " << (pass ? "Rejected ✓" : "Accepted ✗") << "\n";
        if (pass) gen_passed++;
        gen_total++;
        
        // Different lengths
        std::vector<int> lenA = {1, 2, 3};
        std::vector<int> lenB = {1, 2};
        err = validatePermutations(lenA, lenB);
        pass = !err.empty();
        std::cout << "   Different lengths: " << (pass ? "Rejected ✓" : "Accepted ✗") << "\n";
        if (pass) gen_passed++;
        gen_total++;
        
        // Negative values
        std::vector<int> negA = {-1, 0, 1};
        std::vector<int> negB = {1, -1, 0};
        err = validatePermutations(negA, negB);
        pass = !err.empty();
        std::cout << "   Negative values: " << (pass ? "Rejected ✓" : "Accepted ✗") << "\n";
        if (pass) gen_passed++;
        gen_total++;
    }
    
    std::cout << "\n========================================\n";
    std::cout << "GENERALIZED INPUT TEST SUMMARY\n";
    std::cout << "========================================\n";
    std::cout << "Passed: " << gen_passed << " / " << gen_total << " tests";
    if (gen_passed == gen_total) {
        std::cout << " ✓ ALL PASSED!\n";
    } else {
        std::cout << " ✗ SOME FAILED!\n";
    }
    ...
    std::cout << "  ✓ Works with 0-indexed permutations {0..n-1}\n";
    std::cout << "  ✓ Works with non-contiguous value sets\n";
    std::cout << "  ✓ Works with sparse value sets (any non-negative integers)\n";
    std::cout << "  ✓ Properly rejects: duplicates, negatives, mismatched sets\n";

    // ========================================
    // FINAL COMPREHENSIVE RUNTIME SUMMARY
    // ========================================
    std::cout << "\n\n";
    std::cout << "╔══════════════════════════════════════════════════════════════╗\n";
    std::cout << "║           COMPREHENSIVE RUNTIME SUMMARY                      ║\n";
    std::cout << "╠══════════════════════════════════════════════════════════════╣\n";
    std::cout << "║ CONFIGURATION                                                ║\n";
    std::cout << "╠══════════════════════════════════════════════════════════════╣\n";
    printf("║  Array size (n):          %'12d elements                  ║\n", N);
    printf("║  Wavelet tree levels:     %12d levels                    ║\n", timing.levels);
    printf("║  Memory usage:            %12.2f MB                       ║\n", timing.memory_mb);
    printf("║  Batch query count:       %'12d queries                   ║\n", timing.batch_query_count);
    std::cout << "╠══════════════════════════════════════════════════════════════╣\n";
    std::cout << "║ BUILD PERFORMANCE                                            ║\n";
    std::cout << "╠══════════════════════════════════════════════════════════════╣\n";
    printf("║  Build time (CPU→GPU):    %12.3f ms                       ║\n", timing.build_time_ms);
    printf("║  Elements per second:     %12.2f M/s                      ║\n", N / timing.build_time_ms / 1000.0);
    std::cout << "╠══════════════════════════════════════════════════════════════╣\n";
    std::cout << "║ QUERY PERFORMANCE                                            ║\n";
    std::cout << "╠══════════════════════════════════════════════════════════════╣\n";
    printf("║  GPU batch (%d queries):                                   ║\n", timing.batch_query_count);
    printf("║    Total time:            %12.3f ms                       ║\n", timing.gpu_batch_query_time_ms);
    printf("║    Per-query time:        %12.5f µs                       ║\n", timing.gpu_batch_query_time_ms * 1000 / timing.batch_query_count);
    printf("║    Queries per second:    %12.2f M/s                      ║\n", timing.batch_query_count / timing.gpu_batch_query_time_ms / 1000.0);
    std::cout << "║                                                              ║\n";
    printf("║  CPU naive O(n):                                             ║\n");
    printf("║    Per-query time:        %12.5f ms                       ║\n", timing.cpu_per_query_ms);
    printf("║    Estimated batch:       %12.2f ms                       ║\n", timing.cpu_per_query_ms * timing.batch_query_count);
    std::cout << "║                                                              ║\n";
    double speedup = (timing.cpu_per_query_ms * 1000) / (timing.gpu_batch_query_time_ms * 1000 / timing.batch_query_count);
    printf("║  ⚡ SPEEDUP:              %12.1fx                        ║\n", speedup);
    std::cout << "╠══════════════════════════════════════════════════════════════╣\n";
    std::cout << "║ O(log n) VERIFICATION - Query time vs range size            ║\n";
    std::cout << "╠══════════════════════════════════════════════════════════════╣\n";
    std::cout << "║  Range Size       Time (µs)    Result    Status             ║\n";
    std::cout << "║  ──────────────────────────────────────────────────────────  ║\n";
    for (const auto& rt : timing.range_timings) {
        int rsize = std::get<0>(rt);
        double time_us = std::get<1>(rt);
        int result = std::get<2>(rt);
        printf("║  %7d x %-7d  %8.3f  %8d    ✓ O(log n)        ║\n", rsize, rsize, time_us, result);
    }
    std::cout << "║                                                              ║\n";
    if (!timing.range_timings.empty()) {
        double min_time = std::get<1>(timing.range_timings[0]);
        double max_time = std::get<1>(timing.range_timings[0]);
        for (const auto& rt : timing.range_timings) {
            min_time = std::min(min_time, std::get<1>(rt));
            max_time = std::max(max_time, std::get<1>(rt));
        }
        printf("║  Variance: %.2fx (%.3f - %.3f µs) → Confirms O(log n)       ║\n", 
               max_time / min_time, min_time, max_time);
    }
    std::cout << "╠══════════════════════════════════════════════════════════════╣\n";
    std::cout << "║ THEORETICAL ANALYSIS                                         ║\n";
    std::cout << "╠══════════════════════════════════════════════════════════════╣\n";
    printf("║  Wavelet levels (log₂σ):  %12d                         ║\n", timing.levels);
    printf("║  Rank operations/query:   %12d (~4 per level)          ║\n", timing.levels * 4);
    printf("║  CPU complexity:          O(n) = O(%d)                   ║\n", N);
    printf("║  GPU complexity:          O(log n) = O(%d)                   ║\n", timing.levels);
    std::cout << "╚══════════════════════════════════════════════════════════════╝\n";

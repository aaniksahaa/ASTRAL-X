...
__device__ int rank1(const uint64_t* bitvector, const int* block_ranks, int pos) {
    if (pos <= 0) return 0;
    
    int block_idx = pos / BLOCK_SIZE;
    int bit_offset = pos % BLOCK_SIZE;
...
// Rank of 0s
__device__ int rank0(const uint64_t* bitvector, const int* block_ranks, int pos) {
    return pos - rank1(bitvector, block_ranks, pos);
}
...
__global__ void waveletRangeCountKernel(
    ...
    auto countLessThan = [&](int x) -> int {
        ...
        for (int level = levels - 1; level >= 0; level--) {
            ...
            int zeros_before_left = rank0(bv, br, left);
            int zeros_before_right = rank0(bv, br, right + 1);
...
void buildWaveletMatrix(const std::vector<int>& Y, WaveletMatrix& wm) {
    ...
    while ((1 << wm.levels) <= max_val) wm.levels++;
